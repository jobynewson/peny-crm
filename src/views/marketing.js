...
const COLUMNS = [
  ...
  { id: 'planning',   label: 'Planning',         color: 'var(--cat-blue)' },
  { id: 'in-progress',label: 'In Progress',      color: 'var(--cat-amber)' },
  { id: 'scheduled',  label: 'Scheduled / Sent', color: 'var(--cat-green)' },
  { id: 'done',       label: 'Done',             color: 'var(--cat-grey)' },
]

const CARD_TYPES = [
  { id: 'linkedin',     label: 'LinkedIn',      tone: 'blue' },
  { id: 'instagram',    label: 'Instagram',     tone: 'pink' },
  { id: 'mailer',       label: 'Mailer',        tone: 'cyan' },
  { id: 'peny-journal', label: 'Peny Journal',  tone: 'amber' },
  { id: 'ad-hoc',       label: 'Ad-hoc',        tone: 'grey' },
]
...
export class MarketingView {
  ...
  renderKanban(mc) {
    ...
    mountStatusSwitch(mc.querySelector('#mkt-kanban-row'), {
      key: 'marketing', colAttr: 'data-col',
      columns: COLUMNS.map(c => ({ key: c.id, label: c.label, count: this._marketingCardsFor(c.id).length })),
    })
  ...
  renderCard(card) {
    ...
    const duePillColor = daysUntil === null ? '' : daysUntil < 0 ? 'var(--danger)' : daysUntil === 0 ? 'var(--warning)' : 'var(--text-subtle)'
    ...
    <div class="mkt-card" data-card-id="${card.id}"
      ...
      <div style="display:flex;align-items:flex-start;gap:6px;margin-bottom:6px">
        <span style="display:inline-flex;font-size:10px;font-weight:600;padding:2px 7px;border-radius:var(--radius-sm);background:var(--cat-${typeInfo.tone}-soft);color:var(--cat-${typeInfo.tone});flex-shrink:0;line-height:1.5">${typeInfo.label}</span>
      </div>
      ...
      <div style="display:flex;align-items:center;gap:6px;flex-wrap:wrap">
        ${leadUser ? `<span title="${esc(leadUser.name || leadUser.email)}" style="width:20px;height:20px;border-radius:50%;background:var(--cat-blue);display:inline-flex;align-items:center;justify-content:center;font-size:9px;font-weight:700;color:var(--on-cat);flex-shrink:0">${initials(leadUser.name || leadUser.email)}</span>` : ''}
        ${dueLabel ? `<span style="font-size:10px;color:${duePillColor || 'var(--text-tertiary)'};font-weight:500">${dueLabel}</span>` : ''}
        ${subTasks.length ? `<span style="font-size:10px;color:${doneCount === subTasks.length ? 'var(--success)' : 'var(--text-tertiary)'};margin-left:auto">${doneCount}/${subTasks.length}</span>` : ''}
      </div>
      ...
      overlay.innerHTML = `
        <div style="background:var(--bg-primary);border:1px solid var(--border-med);border-radius:var(--radius-lg);width:100%;max-width:560px;box-shadow:var(--shadow-lg);display:flex;flex-direction:column" onclick="event.stopPropagation()">
          ...
          <div style="display:flex;align-items:center;gap:10px;padding:16px 20px;border-bottom:1px solid var(--border-light)">
            <span style="font-size:11px;font-weight:600;padding:3px 9px;border-radius:var(--radius-sm);background:var(--cat-${typeInfo.tone}-soft);color:var(--cat-${typeInfo.tone})">${typeInfo.label}</span>
          ...
          <div style="display:flex;align-items:center;justify-content:space-between;padding:12px 20px">
            ${!isNew ? `<button id="mkt-delete" style="font-size:12px;color:var(--danger);background:none;border:1px solid var(--danger-border);border-radius:var(--radius-sm);padding:6px 14px;cursor:pointer;font-family:var(--font)">Delete card</button>` : '<div></div>'}
